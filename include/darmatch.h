...
class DarMatch {
  ...
  public:
    ...
    std::vector<std::string> seg(const std::string& text, bool forward = true, int max_prefix_matches = 1024) {
      ...
      for (size_t i = 0; i < poslen.size(); i++) {
        words.emplace_back(text.substr(poslen[i].first, poslen[i].second));
      }
      return words;
    }
